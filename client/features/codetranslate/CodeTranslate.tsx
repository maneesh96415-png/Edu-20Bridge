...
function basicSyntaxCheck(lang: string, code: string): string | null {
  ...
  if (lang === "java") {
    if (!/class\s+\w+/.test(code)) return "No class definition found (Java requires a class).";
  }
  if (lang === "c" || lang === "cpp") {
    if (!/int\s+main\s*\(/.test(code)) return "No main function found (expected 'int main(...)').";
  }
  return null;
}
...
export default function CodeTranslatePage() {
  ...
  const runCodeInBrowser = async (lang: string, code: string, setOut: (v: string) => void) => {
    setOut("");
    ...
    if (lang === "javascript" || lang === "typescript") {
      try {
        ...
        console.log = (...args: any[]) => { logs.push(args.map(a => String(a)).join(" ")); origLog(...args); };
        try {
    ...
    if (err) {
      ...
      setOut("Run not available for this language in-browser; syntax looks OK.");
    }
  };
    ...
    <div className="container py-6">
      <div className="rounded-xl border p-4 mb-6 grid gap-4">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <img src="https://cdn.builder.io/api/v1/image/assets%2F3836f58560d244d4a3b3249f2c315c31%2F7b696e140b7644efbfb186147da823e2?format=webp&width=800" alt="CodeTranslate" className="h-12 w-12 rounded-md object-cover shadow-sm" />
            <div>
              <h3 className="text-lg font-semibold">CodeTranslate AI</h3>
              <p className="text-sm text-muted-foreground">Translate and explain code across multiple languages with side‑by‑side editors.</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Button onClick={runTranslation} disabled={loading}>{loading ? "Working…" : (tab === "translate" ? "Translate Code" : "Explain Input Code")}</Button>
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <div className="rounded-lg border overflow-hidden">
          <div className="flex items-center justify-between p-3 border-b">
            ...
            <div className="flex items-center gap-2">
              <Button variant="ghost" onClick={() => runCodeInBrowser(from, input, setLeftRunOutput)}>Run</Button>
              <Button variant="ghost" onClick={() => copyToClipboard(input)}>Copy</Button>
            </div>
          </div>
          <Editor
            height="48vh"
          ...
          <div className="p-3 border-t bg-card">
            ...
            <pre className="mt-2 rounded-md bg-black/90 text-xs text-emerald-300 p-3">{leftRunOutput || "(no output)"}</pre>
          </div>
        </div>
        ...
        <div className="rounded-lg border overflow-hidden">
          <div className="flex items-center justify-between p-3 border-b">
            <div className="flex items-center gap-3">
              ...
              <div className="font-medium">{tab === "translate" ? "Translated Code" : "Explanation"}</div>
              <div className="ml-2 text-sm text-muted-foreground">{tab === "translate" ? to : from}</div>
            </div>
            <div className="flex items-center gap-2">
              <Button variant="ghost" onClick={() => runCodeInBrowser(tab === "translate" ? to : from, output, setRightRunOutput)}>{tab === "translate" ? "Run" : "Analyze"}</Button>
              <Button variant="ghost" onClick={() => copyToClipboard(output)}>Copy</Button>
            </div>
          </div>
          ...
          <div className="p-3 border-t bg-card">
            <div className="text-sm font-medium">Output:</div>
            <pre className="mt-2 rounded-md bg-black/90 text-xs text-emerald-300 p-3">{rightRunOutput || "(no output)"}</pre>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-4">
        <div className="rounded-lg border p-4">
          ...
          <p className="text-sm text-muted-foreground mt-2">Fast overview and line-by-line breakdown will appear here when using the Explain tab.</p>
        </div>
        {tab === "explain" && (
          <div className="rounded-lg border p-4">
            <h4 className="font-semibold">Line-by-Line Breakdown</h4>
            <pre className="mt-2 text-sm text-muted-foreground">{output || "(will show explanation when available)"}</pre>
          </div>
        )}
      </div>
    </div>
